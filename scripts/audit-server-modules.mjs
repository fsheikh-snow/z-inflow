 ...
 * ModuleResolutionException / "x_gzi_zscaler_ppm is not defined" on the instance.
 *
 ...
 * `new x_gzi_zscaler_ppm.Foo()` only works inside Class.create Script Include files.
 */
...
const scopedGlobalNew = /\bnew\s+x_gzi_zscaler_ppm\./g
...
for (const file of walk(serverRoot)) {
    ...
    if (!isScriptIncludeClass && scopedGlobalNew.test(text)) {
        errors.push(
            `${rel}: use import { X } from '@servicenow/glide/x_gzi_zscaler_ppm' — new x_gzi_zscaler_ppm.X() is undefined in ES modules`
        )
    }
}
...
console.log('Server module audit OK (relative imports use extensions; ES modules avoid x_gzi_zscaler_ppm.* globals).')
