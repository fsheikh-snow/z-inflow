...
- `GET /api/x_gzi_zscaler_ppm/v1/portfolios`
- `GET /api/x_gzi_zscaler_ppm/v1/projects`
- `GET /api/x_gzi_zscaler_ppm/v1/users/search?q=a`
- `GET /x_gzi_zscaler_ppm_workspace.do` — fails on HTTP 5xx or body text `PM Workspace failed`

After creating a project/portfolio, optionally verify members:

- `GET /api/x_gzi_zscaler_ppm/v1/projects/{id}/members`
- `GET /api/x_gzi_zscaler_ppm/v1/portfolios/{id}/members`
...
`npm run smoke` / `npm run ship` first run **`npm run audit:modules`** so extensionless relative imports and illegal `new x_gzi_zscaler_ppm.*` in ES modules fail before hitting the instance.
...
| `"x_gzi_zscaler_ppm" is not defined` | ES module used `new x_gzi_zscaler_ppm.UserService()` | `import { UserService } from '@servicenow/glide/x_gzi_zscaler_ppm'` |
